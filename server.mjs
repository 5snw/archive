...
const BANDCAMP_TRACK_API = "https://bandcamp.com/api/mobile/24/tralbum_details?band_id=2023554063&tralbum_type=t&tralbum_id=3478113475";
...
async function redirectMusic(response) {
    const upstream = await fetch(BANDCAMP_TRACK_API);
    if (!upstream.ok) throw new Error(`Bandcamp metadata failed: ${upstream.status}`);
    const release = await upstream.json();
    const streamUrl = release?.tracks?.[0]?.streaming_url?.["mp3-128"];
    if (!streamUrl || !streamUrl.startsWith("https://bandcamp.com/stream_redirect?")) {
        throw new Error("Bandcamp stream unavailable");
    }
    response.writeHead(307, { "Location": streamUrl, "Cache-Control": "no-store, max-age=0" });
    response.end();
}
...
const server = createServer(async (request, response) => {
    try {
        ...
        if (url.pathname === "/api/music/aonde-mais") {
            if (request.method !== "GET" && request.method !== "HEAD") {
                response.writeHead(405, { "Allow": "GET, HEAD" }).end();
                return;
            }
            await redirectMusic(response);
            return;
        }
        if (request.method !== "GET" && request.method !== "HEAD") {
            response.writeHead(405, { "Allow": "GET, HEAD" }).end();
